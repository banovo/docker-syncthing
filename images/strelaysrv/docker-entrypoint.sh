...
CONTAINER_NAME=${CONTAINER_NAME:-strelaysrv}
...
# STARTING
# If command starts with an option, prepend strelaysrv
# -----------------------------------------------------------------------------
if [ "${1:0:1}" = '-' ]; then
	set -- strelaysrv "$@"
fi

# If command starts with strelaysrv, run as container user
# -----------------------------------------------------------------------------
if [ "$1" = 'strelaysrv' ]; then
